...
class SageMakerValidator():

    def get_int(self, param_value, param_name):
        try:
    ...
    def get_float(self, param_value, param_name):
        try:
    ...
    def validate_hyperparameters(self, cfg):
        warnings = 0
        ...
        if "min_count" in cfg:
            tmp = self.get_int(cfg["min_count"], "min_count")
            if tmp < 0:
                raise Exception(
                    "Parameter 'min_count' should be >= 0.")

        if "sampling_threshold" in cfg:
            tmp = self.get_float(cfg["sampling_threshold"], "sampling_threshold")
            if tmp <= 0 or tmp >= 1:
                raise Exception(
                    "Parameter 'sampling_threshold' should be between (0,1)")

        if "learning_rate" in cfg:  # Default: .05
            tmp = self.get_float(cfg["learning_rate"], "learning_rate")
            if tmp <= 0:
                raise Exception(
        ...
        if "window_size" in cfg:  # Default: 5
            ws = self.get_int(cfg["window_size"], "window_size")
            if ws <= 0:
                raise Exception(
                    "Parameter 'window_size' should be > 0.")

        if "vector_dim" in cfg:  # Default: 100
            tmp = self.get_int(cfg["vector_dim"], "vector_dim")
            if tmp <= 0:
                raise Exception(
    ...
    def validate_params(self, resource_config, hyperparameters):
        ...
        warnings = self.validate_hyperparameters(hyperparameters)
