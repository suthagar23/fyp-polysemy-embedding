...
# on multiple servers.
INPUT_BASE_PATH="/media/suthagar/Data/Corpus/Sep29/swivel/preprocessed"

# Where the output and working directory is located.
OUTPUT_BASE_PATH="/media/suthagar/Data/Corpus/Sep29/swivel/embedding-gpu"

# Location of evaluation data, if you want to observe evaluation while training.
EVAL_BASE_PATH="/home/suthagar/PycharmProjects/Polysemy/swivel/eval"
...
python swivel.py --job_name worker --task_index 1 --gpu_device 0 ${ARGS} >& /tmp/worker.1 &
python swivel.py --job_name worker --task_index 2 --gpu_device 0 ${ARGS} >& /tmp/worker.2 &
python swivel.py --job_name worker --task_index 3 --gpu_device 0 ${ARGS} >& /tmp/worker.3 &
